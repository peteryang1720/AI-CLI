## ⚠️ 免责声明（Disclaimer）
本项目 **`AI-CLI`** 是一个基于大语言模型的实验性命令行辅助工具，旨在将自然语言指令转换为 Shell 命令以提升终端操作效率。尽管项目内置了安全过滤机制（如高危命令拦截、执行前用户确认等），但**无法保证 100% 的安全性或准确性**。
使用本工具即表示您理解并同意以下条款：
1. **风险自担**  
...
4. **作者不承担任何责任**  
   项目作者及贡献者**不对因使用本软件所导致的任何直接或间接损失（包括但不限于数据丢失、系统损坏、服务中断等）承担责任**。
5. **安全依赖用户判断**  
